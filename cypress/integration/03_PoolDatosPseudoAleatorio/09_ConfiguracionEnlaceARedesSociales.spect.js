...
describe('Prueba la pagina ghost', () => {
    ...
    (async function () {
        try {
            const response = await fetch('https://my.api.mockaroo.com/redesSociales1.json?key=292d8a80');
            let  myJson = await response.json();
            for (let i=0; i< myJson.length; i++) {
                //console.log(myJson[i]);
                arrayUrls.push(myJson[i]["url"])
            }
    ...
    function random() {
        let number = Math.floor(Math.random()*arrayUrls.length);
        console.log(number)
        return number
       }

    let posicion = random()
    let valor = arrayUrls[posicion]
