...
describe(`Pruebas con Ghost - Faker`, function() {
    ...
    let arraySentencia = new Array;

    (async function () {
        try {
            const response = await fetch('https://my.api.mockaroo.com/tagname_sentencia.json?key=292d8a80');
            let  myJson = await response.json();
            for (let i=0; i< myJson.length; i++) {
                //console.log(myJson[i]);
                arraySentencia.push(myJson[i]["tag-name"])
            }
    ...
    function random() {
        let number = Math.floor(Math.random()*arraySentencia.length);
        console.log(number)
        return number
       }

    let posicion = random()
    let valor = arraySentencia[posicion]
    ...
    /*Escenario 4*/
    it(`Crear tag con sentencia`, function() { 
