...
describe(`Pruebas con Ghost - Faker`, function() {
    ...
    (async function color() {
        try {
            const response = await fetch('https://my.api.mockaroo.com/tagcolor.json?key=292d8a80');
            let  myJson = await response.json();
            for (let i=0; i< myJson.length; i++) {
                //console.log(myJson[i]);
                arrayColores.push(myJson[i]["tag-color"])
            }
    ...
    (async function descripcion() {
        try {
            const response = await fetch('https://my.api.mockaroo.com/tagdescripcion.json?key=292d8a80');
            let  myJson = await response.json();
            for (let i=0; i< myJson.length; i++) {
                //console.log(myJson[i]);
                arrayDescripcion.push(myJson[i]["tag-color"])
            }
    ...
    let posicion = random()
    let valorNombre = arrayNombre[posicion]
    let valorColor = arrayColores[posicion]
    color = valorColor.substr(1)

    let valorDescripcion = arrayDescripcion[posicion]
    ...
    it(`Crear tag con descripcion cantidad maxima aceptable`, function() { 
        ...
        cy.get('input[id="tag-name"]').click({force:true}).type(valorNombre)
        cy.get('input[id="tag-slug"]').click({force:true}).clear()
        cy.get('input[id="tag-slug"]').click({force:true}).type(valorNombre)
        cy.get('input[name="accent-color"]').click({force:true}).type(color)
        cy.get('#tag-description').click({force:true}).type(valorDescripcion)
